...
object day6 {
  ...
  class InstructionParser extends JavaTokenParsers {
    def point: Parser[(Int, Int)] = {wholeNumber ~ "," ~ wholeNumber ^^
      { case x ~ "," ~ y => (x.toInt, y.toInt) }}
    def square: Parser[Square] = point ~ "through" ~ point ^^
      { case p1 ~ "through" ~ p2 => Square(p1._1 to p2._1, p1._2 to p2._2) }
    def lighting: Parser[Lighting] =
      "turn off" ^^ {case _ => TurnOff} |
      "turn on" ^^ {case _ => TurnOn} |
      "toggle" ^^ {case _ => Toggle}
